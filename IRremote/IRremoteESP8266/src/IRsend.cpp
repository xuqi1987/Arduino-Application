...
uint16_t IRsend::mark2(uint32_t time) {
  // Handle the simple case of no required frequency modulation.
  if (!modulation || _dutycycle >= 100) {
    ledOn();
    if (time < 16384)
    ...
    {
      uint32_t v = time / 16384;
      uint32_t m = time % 16384;

      for (int i = 0; i < v; i++)
      {
        _delayMicroseconds(16384);
      }
      _delayMicroseconds(m);
    }
  ...
  while (elapsed < time) {  // Loop until we've met/exceeded our required time.
    ...
    // Calculate how long we should pulse on for.
    // e.g. Are we to close to the end of our requested mark time (usec)?
    _delayMicroseconds(std::min((uint32_t) onTimePeriod, time - elapsed));
    ledOff();
    counter++;
    if (elapsed + onTimePeriod >= time)
      return counter;  // LED is now off & we've passed our allotted time.
    // Wait for the lesser of the rest of the duty cycle, or the time remaining.
    _delayMicroseconds(std::min(time - elapsed - onTimePeriod,
                                (uint32_t) offTimePeriod));
    elapsed = usecTimer.elapsed();  // Update & recache the actual elapsed time.
  }
  return counter;
...
// Args:
//   time: Time in microseconds (us).
void IRsend::space2(uint32_t time) {
  ledOff();
  if (time == 0) return;
...
void IRsend::sendRaw2(uint32_t buf[], uint16_t len, uint16_t hz){
